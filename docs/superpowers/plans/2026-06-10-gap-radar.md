...
Add the new `ISignalSnapshotRepo` interface:
```ts
...
In the `beforeEach` setup SQL block, add after the existing `CREATE TABLE` statements:
...
And update the `opportunities` CREATE TABLE to include `gap_score REAL` column (add it after `alerted_at TEXT`).
...
After the `runScore(...)` call in the cron handler:

```ts
// Gap snapshot + scoring (runs after runScore so gap_score is fresh)
const d1repoGap = new D1Repo(env.DB);
await runSnapshot(d1repoGap, d1repoGap);
await runGapScore(d1repoGap, d1repoGap);
...
In the HTTP router in `index.ts`, add this route (alongside other GET routes):
...
if (method === 'GET' && path === '/gap-radar') {
  ...
  const entries = rows.map(r => ({
    ...
    momentum:       null, // computed client-side from trend — not stored on row
    solution_ratio: r.solution_ratio,
...
Note: `momentum` is not stored directly in the DB query — omit it for now; the dashboard will display `▲/→/▼` only when we have prior-week data. Simplify to returning `gap_score` directly.

Actually, update the return to drop momentum (it requires a second query per segment that's expensive). The frontend will just show the gap_score and its columns without a separate momentum percentage:
...
export async function collectAppSumo(keywords: string[], segment: string): Promise<Signal[]> {
  ...
  // Extract deal titles from og:title or h2/h3 tags (basic heuristic)
  const titleMatches = [...html.matchAll(/<h2[^>]*class="[^"]*product[^"]*"[^>]*>([\s\S]*?)<\/h2>/gi)];
...
<script lang="ts">
  ...
  export let segment: string | null = null; // opened DeployModal for this segment
...
4. Add the tab panel (alongside the existing `{#if activeTab === ...}` blocks):
```svelte
{#if activeTab === 'radar'}
  <GapRadar
...
Where `deploySegment` and `showDeploy` are the existing variables that control `DeployModal`.
